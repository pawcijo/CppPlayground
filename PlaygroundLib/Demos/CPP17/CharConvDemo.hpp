...
private:
  void PerformanceTest()
  {
    ...
    auto stoiTime =
      measureTime([this](const std::string& s) { StoiTimeTest(s); }, str);
    auto stringstreamTime = measureTime(
      [this](const std::string& s) { StringstreamTimeTest(s); }, str);

    auto minTime = std::min({ fromCharsTime, stoiTime, stringstreamTime });

    auto maxTime = std::max({ fromCharsTime, stoiTime, stringstreamTime });

    auto printTimeWithColor =
      [](const char* name, auto time, auto minTime, auto maxTime)
    {
      if (time == minTime)
      {
        std::cout << GREEN_TEXT_START << name << ": " << GREEN_BOLD_TEXT_START
                  << time << " ns" << TEXT_FORMAT_CLEAR << std::endl;
      }
      else if (time == maxTime)
      {
        std::cout << RED_TEXT_START << name << ": " << RED_BOLD_TEXT_START
                  << time << " ns" << TEXT_FORMAT_CLEAR << std::endl;
      }
      else
      {
        std::cout << YELLOW_TEXT_START << name << ": " << YELLOW_BOLD_TEXT_START
                  << time << " ns" << TEXT_FORMAT_CLEAR << std::endl;
      }
    };
