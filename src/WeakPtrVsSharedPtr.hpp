...
namespace WeakPtrVsSharedPtr
{
    ...
    std::vector<std::pair<std::string, std::string>> Notes = {{{"Purpose"},
                                                              ...
                                                              {{"No Reference Count"},
                                                               {"A std::weak_ptr does not affect the reference count of the shared_ptr it refers to. This \n"
                                                                "helps avoid circular references (a situation where two or more shared_ptrs reference each other, \n"
                                                                "preventing the reference count from ever reaching zero and causing memory leaks). \n"}},
                                                              {{"Expiration"},
                                                               {"Since a std::weak_ptr does not own the object, it can become expired when the last std::shared_ptr \n"
                                                                "holding the object is destroyed. Before accessing the object, you must check if the weak_ptr \n"
    ...
    public:
        ExampleClass() : variable(5), another_variable(5.0001f) {};

        int Variable() { return variable; }
